...
SCRIPT_NAME = 'ircrypt'
SCRIPT_AUTHOR = \
    'Sven Haardiek <sven@haardiek.de>, Lars Kiesow <lkiesow@uos.de>'
...
def ircrypt_decrypt_hook(data, msgtype, server, args):
    ...
    if '>CRY-' not in args:
        # if key exisits and no >CRY not part of message flag message as
        # unencrypted
...
def ircrypt_encrypt_hook(data, msgtype, server, args):
    ...
    try:
        ...
        inp = key + b'\n' + message
    (ret, out, err) = ircrypt_gnupg(
        inp, '--symmetric', '--cipher-algo', cipher, '--passphrase-fd', '-')
...
def ircrypt_config_init():
    ...
    ircrypt_config_file = weechat.config_new(
        'ircrypt', 'ircrypt_config_reload_cb', '')
    ...
    ircrypt_config_option['unencrypted'] = weechat.config_new_option(
        ...
        'string',
        'Marker for unencrypted messages received in an encrypted channel',
        '', 0, 0, '', 'u', 0, '', '', '', '', '', '')
...
def ircrypt_config_reload_cb(data, config_file):
    ...
    # Forget Keys and ciphers to make sure they are properly reloaded and no
    # old ones are left
    ircrypt_keys = {}
    ircrypt_cipher = {}
...
def ircrypt_command_remove_cip(target):
    ...
    try:
        ...
        ircrypt_info(
            'Removed special cipher. Using default cipher for %s instead.' %
            target)
    except KeyError:
...
def ircrypt_encryption_statusbar(*args):
    ...
    channel = weechat.buffer_get_string(
        weechat.current_buffer(), 'localvar_channel')
    server = weechat.buffer_get_string(
        weechat.current_buffer(), 'localvar_server')
...
if __name__ == '__main__' and weechat.register(
    ...
    weechat.hook_signal('ircrypt_buffer_opened',
