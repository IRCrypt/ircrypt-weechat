...
def ircrypt_gpg_init():
    ...
    # Probe for GPG key
    (ret, out, err) = ircrypt.ircrypt_gnupg(
        b'', '--homedir', ircrypt_gpg_homedir,
    ...
    if out:
        try:
            ircrypt_gpg_id = out.decode(
                'utf-8').split('fpr')[-1].split('\n')[0].strip(':')
    ...
    ircrypt.ircrypt_warn(
        'No private key for assymetric encryption was found in the '
        'IRCrypt GPG keyring. IRCrypt will now try to automatically generate '
        'a new key. This might take quite some time as this procedure depends '
        'on the gathering of enough entropy for generating cryptographically '
    ...
    binary = weechat.config_string(
        weechat.config_get('ircrypt.general.binary'))
...
def ircrypt_key_generated_cb(data, command, errorcode, out, err):
    ...
    ircrypt.ircrypt_info(
        'A private key for asymmetric encryption was successfully'
...
def ircrypt_receive_key_ex_ping(server, args, info):
    ...
    if not ircrypt_gpg_id:
        ircrypt.ircrypt_error(
            'Error in IRCrypt key exchange', weechat.current_buffer())
        weechat.command('', '/mute -all notice -server %s %s '
                        '>UCRY-INTERNAL-ERROR' % (server, info['nick']))
        return ''
...
def ircrypt_public_key_send(server, nick):
    ...
    for i in range(1 + (len(pub_key) // MAX_PART_LEN))[::-1]:
        msg = '>PUB-EX-%i %s' % (
            i,
            pub_key[i * MAX_PART_LEN:(i + 1) * MAX_PART_LEN])
        weechat.command('', '/mute -all notice -server %s %s %s' %
                        (server, nick, msg))
...
def ircrypt_public_key_get(server, args, info):
    ...
    if int(number):
        if target not in ircrypt_pub_keys_memory:
    ...
    if not ircrypt_key_ex_memory[target].pub_key_receive:
        ircrypt.ircrypt_error(
            '%s sends his public key without inquiry' % info['nick'],
    ...
    # Import public key
    (ret, out, err) = ircrypt.ircrypt_gnupg(
        message, '--homedir', ircrypt_gpg_homedir,
...
def ircrypt_sym_key_send(server, nick):
    ...
    (ret, out, err) = ircrypt.ircrypt_gnupg(
        keypart, '--homedir', ircrypt_gpg_homedir,
    ...
    if ircrypt_key_ex_memory[target].parts == 2:
        ...
        # If the symmetric key is also complete by the counterpart set
        # symmetric key
        if ircrypt_key_ex_memory[target].sym_received:
            weechat.command(
                '', '/ircrypt set-key -server %s %s %s'
                % (server, info['nick'],
    ...
    for i in range(1 + (len(out) / MAX_PART_LEN))[::-1]:
        msg = '>SYM-EX-%i %s' % (i,
                                 out[i * MAX_PART_LEN:(i + 1) * MAX_PART_LEN])
        weechat.command('', '/mute -all notice -server %s %s %s' %
                        (server, nick, msg))
...
def ircrypt_sym_key_get(server, args, info):
    ...
    if int(number) != 0:
        if catchword not in ircrypt_sym_key_memory:
    ...
    if (ircrypt_key_ex_memory[target].pub_key_send or
        ...
        ircrypt.ircrypt_error(
            '%s sends symmetric key without inquiry' % info['nick'],
            weechat.current_buffer())
        weechat.command(
            '', '/mute -all notice -server %s %s '
...
def ircrypt_config_init():
    ...
    ircrypt_config_file = weechat.config_new(
        'ircrypt-keyex', 'ircrypt_config_reload_cb', '')
...
def ircrypt_config_asym_id_read_cb(data, config_file, section_name,
                                   option_name, value):
...
def ircrypt_command_start(server, nick):
    ...
    if gpg_id:
        weechat.command(
            '', '/mute -all notice -server %s %s >KEY-EX-PING %s %s'
    ...
    # print information
    ircrypt.ircrypt_info(
        'Start key exchange with %s on server %s. This may take some '
...
def ircrypt_command_remove_public_key(target):
    '''ircrypt command to remove public key for target
    ...
    if target.lower() not in ircrypt_asym_id:
        ircrypt.ircrypt_error('No existing public key for %s.' %
                              target, weechat.current_buffer())
        return weechat.WEECHAT_RC_ERROR
    ...
    if ret:
        ircrypt.ircrypt_error(
            'Could not delete public key in gpg', weechat.current_buffer())
        return weechat.WEECHAT_RC_ERROR
    elif err:
        ircrypt.ircrypt_warn(err.decode('utf-8'))
...
def ircrypt_command(data, buffer, args):
    ...
    if argv and not argv[0] in ['list', 'remove-public-key', 'start']:
        ircrypt.ircrypt_error(
            '%sUnknown command. Try  /help ircrypt-keyex', buffer)
        return weechat.WEECHAT_RC_ERROR
    ...
    if not server:
        ...
        ircrypt.ircrypt_error(
            'Unknown Server. Please use -server to specify server', buffer)
        return weechat.WEECHAT_RC_ERROR
...
def ircrypt_notice_hook(data, msgtype, server, args):
    ...
    if '>UCRY-INTERNAL-ERROR' in args:
        ircrypt.ircrypt_error(
            '%s on server %s reported an error during the key exchange'
        ...
        ircrypt.ircrypt_error(
            '%s on server %s reported that your fingerprint known does'
            'not match his own fingerprint' % (
                info['nick'], server),
            weechat.current_buffer())
        return ''
...
def ircrypt_init():
    ...
    if weechat.config_string(weechat.config_get('ircrypt.general.binary')):
        ...
        weechat.hook_command(
            'ircrypt-keyex', 'Commands of the Addon IRCrypt-keyex',
            '[list] '
